...
describe Session do
  ...
  let(:cook) { WEBrick::Cookie.new('_minirails_app', { xyz: 'abc' }.to_json) }
  ...
  describe "#store_session" do
    context "without cookies in request" do
      ...
      it "adds new cookie with '_minirails_app' name to response" do
    ...
    context "with cookies in request" do
      before(:each) do
        cook = WEBrick::Cookie.new('_minirails_app', { pho: "soup" }.to_json)
        req.cookies << cook
      end
